...
const CartPage = () => {
  ...
  const getTotalPrice = () => {
    return cart.cartItems.reduce((total, item) => total + parseFloat(item.price.slice(1)) * item.quantity, 0);
  };

  const handleIncrement = (itemId) => {
    cart.setCartItems(prevCartItems =>
      prevCartItems.map(item =>
        item.id === itemId ? { ...item, quantity: item.quantity=='NaN'?'0':item.quantity + 1 } : item
      )
    );
  };
          ...
          {cart.cartItems.map((item) => (
            <div key={item.id} className="flex items-center mb-4">
              <div className="flex-shrink-0 w-16 h-16 bg-primary rounded-lg overflow-hidden">
                <img
                  src={item.imgsrc || db.products.find(p => p.name === item.name)?.imgsrc || 'https://via.placeholder.com/150'}
